...
def argument_parse():
    ...
    parser.add_argument(
        ...
        help="background Threshold pixel cutoff area percent",
        required="True",
    )
    parser.add_argument(
        "-t", "--threshold", help="background Threshold pixel cutoff", default="245"
    )
    parser.add_argument(
        "-m", "--threshold_mean", help="background Threshold mean cutoff", default="245"
    )
    parser.add_argument(
        "-d", "--threshold_std", help="background Threshold std cutoff", default="0"
    )
    parser.add_argument(
        "-z", "--patch_byte_cutoff", help="patch_byte_cutoff", default="0"
    )
    return parser
...
def create_summary_img(
    ...
    for i in range(0, len(patch_stop_x_list), 1):
        x1 = int(
            (patch_start_x_list[i] * OSobj.level_downsamples[patch_level]) / toplevel[2]
        )
        x2 = int(
            (patch_stop_x_list[i] * OSobj.level_downsamples[patch_level]) / toplevel[2]
        )
        y1 = int(
            (patch_start_y_list[i] * OSobj.level_downsamples[patch_level]) / toplevel[2]
        )
        y2 = int(
            (patch_stop_y_list[i] * OSobj.level_downsamples[patch_level]) / toplevel[2]
        )
        poly_included.append(
    ...
    f.savefig(
        os.path.join(patch_dir, samp + "_mask_patchoverlay.png"), pad_inches="tight"
    )
    return True
...
def patch_feature_extraction(
    image_string, res50, adaptive_mean_spatial_layer, input_shape=(256, 256, 3)
):
    """
    Args:
...
def create_tfrecord(
    ...
    for i in range(0, len(patch_start_x_list), 1):
        ...
        image_feature = patch_feature_extraction(
            img, res50, adaptive_mean_spatial_layer, (patch_size, patch_size, 3)
        )
        image_format = "jpeg"
        """writing tfrecord"""
        feature = {
    ...
    f.set_size_inches(patch_sub_size_x / DPI, patch_sub_size_y / DPI)
    f.savefig(
        os.path.join(patch_dir, samp + "_mask_patchoverlay_final.png"),
        pad_inches="tight",
    )
...
def main():
    ...
    print(
        "Entered background Threshold pixel cutoff area percent "
        + arg.threshold_area_percent
    )
    ...
    """creating binary mask to inspect areas with tissue and performance of threshold"""
    list_binary_img, toplevel = create_binary_mask_new(
        rgb2hed_thresh, svs_file, patch_dir, samp
    )
    """extracting patch coordinates for requested level based on threshold"""
    (
        patch_start_x_list,
