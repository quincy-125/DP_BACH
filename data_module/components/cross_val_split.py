...
def cross_val_data(neg_slides, pos_slides, kf_csv_path, test_ratio=0.0, n_folds=5, kf_shuffle=False, kf_rs=None):
    """_summary_
    ...
    if test_ratio != 0.0:
        ...
        test_df.to_csv(
            os.path.join(kf_csv_path, "bach_ratio_{}_p_{}_test.csv".format(
    ...
    for f in range(n_folds):
        ...
        val_fold_df.to_csv(
            os.path.join(kf_csv_path, "bach_fold_{}_val.csv".format(f+1)), 
            index=False
        )
...
def run_kf_cross_val(
    ...
    logging.info("Starting to Execute {}-Fold Cross Validation Data Split".format(n_folds))
    cross_val_data(
        neg_slides=neg_slides, 
        pos_slides=pos_slides, 
    ...
    logging.info("Successfully Completed the Execution of {}-Fold Cross Validation Data Split".format(n_folds))